...
└── Makefile           # make setup / run / dashboard / upload
```
...
The hub serves the UI's data, so the simplest setup is to point a browser at the
hub and serve the static `frontend/` from the same origin (or any static server
and use `?backend=`):

```bash
# 1. start the existing hub (needs the ESP32 over BLE)
...
With no hub reachable the UI runs its built-in simulation, so you can open
`frontend/index.html` on its own to explore the interface.
